...
import content from "@/data/soiMoiContent.json";
...
const IMG_DIR = `${BASE}articles/soi-moi/`;
      ...
      {indices.map((idx) => {
        ...
        return (
          <ArticleImg
            key={idx}
            src={`${IMG_DIR}${file}`}
            alt={`Illustration ${idx} — Différence entre le Moi et le Soi`}
            maxHeight={420}
          />
        );
      })}
  ...
  if (block.type === "img") {
    ...
    return (
      <ArticleImage
        key={key}
        src={`${IMG_DIR}${file}`}
        alt={`Illustration ${block.index} — Différence entre le Moi et le Soi`}
      />
    );
  }
...
export default function ArticleSoiMoi() {
